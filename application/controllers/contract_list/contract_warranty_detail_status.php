...
class Contract_warranty_detail_status extends MY_Controller {

	public function __construct() {
        parent::__construct();
        $this->load->model(array('contract_warranty_detail_status_model'));
    }

    public function index() {
    	$data_send = array(
            'page_title' => ci_trans('Contract warranty status')
        );

        $this->page_render($data_send);
    }

    public function get_contract_warranty_detail_status_list() {
        ...
        $result_list 
        	= $this->contract_warranty_detail_status_model
        		->get_contract_warranty_detail_status_list($start, $length, $search);
    ...
    public function get_contract_warranty_detail_status_by_id() {
        ...
        $result = $this->contract_warranty_detail_status_model
            ->get_data_from_specify_field_value($primary_field, $id, $this->contract_warranty_detail_status_model->base_table);

		echo json_encode($result);
    }

    public function save_contract_warranty_detail_status() {
    	...
    	$data_check_for_mandatory_field = array(
            'status' => ci_trans('please_complete_information').' : '.ci_trans('Contract warranty status')
        );
        ...
        $result_simple_check_is_duplicated = $this->contract_warranty_detail_status_model->simple_check_is_duplicated($post['id'], 
            array(
                'status' => $post['status']
            )
        );
        ...
        $result_save = $this->contract_warranty_detail_status_model->save($post);
